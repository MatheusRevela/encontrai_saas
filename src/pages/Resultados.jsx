...
import { StartupRepo, TransacaoRepo, UserRepo } from '@/components/services/entityRepository';
  ...
  const { data: transacao, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!currentTransacao.startups_sugeridas?.length) {
        ...
        } catch { /* se falhar, segue sem exclusão */ }

        const startupsParaLLM = prefiltrarStartups(problemaCompleto, todasStartups, idsJaComprados);
        ...
        currentTransacao = await TransacaoRepo.update(currentTransacao.id, {
