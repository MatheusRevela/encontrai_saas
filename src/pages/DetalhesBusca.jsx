                      ...
                      <Card key={`${idx}-${startup.startup_id}`} className="bg-white border-0 shadow-lg overflow-hidden">
                        <CardContent className="p-6">
                          ...
                          <div className="bg-slate-50 rounded-xl p-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                              {startup.email && <div className="flex items-center gap-2"><Mail className="w-4 h-4 text-slate-500" /> <a href={`mailto:${startup.email}?subject=Conheci vocês através do EncontrAI&body=Olá ${startup.nome},%0D%0A%0D%0AEncontrei a solução de vocês através do EncontrAI e gostaria de saber mais.${busca.dor_relatada ? `%0D%0A%0D%0AMinha necessidade:%0D%0A${encodeURIComponent(busca.dor_relatada)}` : ''}`} className="text-blue-600 hover:underline">{startup.email}</a></div>}
                              {startup.whatsapp && <div className="flex items-center gap-2"><Phone className="w-4 h-4 text-slate-500" /> <a href={`https://wa.me/${startup.whatsapp.replace(/\D/g,'')}?text=Olá ${startup.nome}! Encontrei a solução de vocês através do EncontrAI.${busca.dor_relatada ? `%0A%0AMinha necessidade: ${encodeURIComponent(busca.dor_relatada)}` : ''}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">{startup.whatsapp}</a></div>}
